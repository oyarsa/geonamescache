...
class GeonamesCache:
    ...
    def search_cities(self, query, attribute="alternatenames", case=True):
        """Search all city records and return list of records that match
        query for given attribute.

        case: bool, default True
            If True, comparison between `query` and the record is case-sensitive.
        """

        def check(query: str, record: Dict[str, Iterable[str]]) -> bool:
            if case:
                return query in record[attribute]
            return query.lower() in map(str.lower, record[attribute])

        results = []
        for record in self.get_cities().values():
            if check(query, record):
